...
document.addEventListener('scroll', function() {
  ...
  const fillerOffsetTop = filler.offsetTop;
  const fillerHeight = filler.offsetHeight;
  const scrollTop = window.scrollY;
  const scrollTopRelativeToFiller = scrollTop - fillerOffsetTop;
  const sections = document.querySelectorAll('.section');
  const totalSections = sections.length;
  const totalScrollPercent = Math.min(Math.max(scrollTopRelativeToFiller / fillerHeight, 0), 1);
